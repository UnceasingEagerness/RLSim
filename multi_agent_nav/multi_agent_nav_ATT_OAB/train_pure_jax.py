...
def main():
    ...
    os.makedirs("logs_stae", exist_ok=True)
    os.makedirs("checkpoints_stae", exist_ok=True)
    ...
    with console.status("[bold yellow]Saving Model Checkpoint and Metrics to disk...[/bold yellow]"):
        ...
        metrics_df.to_csv("logs_stae/metrics.csv", index=False)
        
        # Save Network Weights
        ckpt = PyTreeCheckpointer()
        actor_path = os.path.abspath("checkpoints_stae/sac_actor_final")
        critic_path = os.path.abspath("checkpoints_stae/sac_critic_final")
    ...
    console.print("[bold green]✔ Saved metrics to logs_stae/metrics.csv[/bold green]")
    console.print("[bold green]✔ Saved model weights to checkpoints_stae/[/bold green]")
