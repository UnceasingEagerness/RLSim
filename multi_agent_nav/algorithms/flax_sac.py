...
class CentralizedSoftQNetwork(nn.Module):
    """CTDE Critic: Takes global joint state and joint actions, outputs global Q-value."""
    @nn.compact
    def __call__(self, global_x, global_a):
        # global_x: [B, N, obs_dim]
        # global_a: [B, N, action_dim]
        B = global_x.shape[0]
        
        # Flatten all agents' observations and actions together
        x_flat = global_x.reshape((B, -1))
        a_flat = global_a.reshape((B, -1))
        
        x = jnp.concatenate([x_flat, a_flat], axis=1)
        
        # Larger capacity because it's processing N times more data
        x = nn.Dense(512)(x)
        x = nn.relu(x)
        x = nn.Dense(512)(x)
        x = nn.relu(x)
        x = nn.Dense(256)(x)
        x = nn.relu(x)
        # Output a single global Q-value for the joint action
        x = nn.Dense(1)(x) 
        return x
...
class FlaxSpatioTemporalAttentionEncoder(nn.Module):
    ...
    def __call__(self, ego_seq, entity_seqs):
        ...
        if num_entities == 0:
            ...
            # sow_weights=True saves the attention weights into Flax's 'intermediates' collection
            attn_output = nn.MultiHeadDotProductAttention(num_heads=4, qkv_features=self.hidden_dim, deterministic=True)(
                inputs_q=ego_q, inputs_kv=ent_kv, mask=key_mask, sow_weights=True
            )
...
class STAE_Max_ActorBackbone(nn.Module):
    ...
    def __call__(self, x_flat):
        B = x_flat.shape[0]
        obs_dim = x_flat.shape[-1] // self.seq_len
        x_seq = x_flat.reshape((B, self.seq_len, obs_dim))
