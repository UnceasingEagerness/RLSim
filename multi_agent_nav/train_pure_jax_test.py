...
def main():
    ...
    if ENCIRCLE_MODE:
        env_params = env_params.replace(
            ...
            num_obstacles           = 200,    # reduced from 600 — easier start for new behavior
        )
    # ─────────────────────────────────────────────────────
    ...
    if ENCIRCLE_MODE:
        ckpt_dir = "checkpoints_encircle"
        log_dir  = "logs_encircle"
    else:
    ...
    if os.path.exists(actor_path) and os.path.exists(critic_path):
        console.print(f"[bold cyan]Resuming from {ckpt_dir}/ checkpoint...[/bold cyan]")
        actor_params  = ckpt.restore(actor_path,  item=actor_params)
        critic_params = ckpt.restore(critic_path, item=critic_params)
        ...
        actor_params  = ckpt.restore(warmstart_actor,  item=actor_params)
        critic_params = ckpt.restore(warmstart_critic, item=critic_params)
    else:
